    ...
    steps:
      ...
      - name: Generate M3U Playlists
        run: |
          ...
          # Filter output, keeping only valid #EXTINF and URL pairs
          cat en/raw_output.txt | grep -v -E '^<|^$|^\s*$|github\.com.*offline\.mp4' | \
          awk '/^#EXTINF/ {if (p) print p; p=$0; next} p && !/^#/ && !/github\.com/ && /\.m3u8$/ {print p; print; p=""} END {if (p) print "# Skipped incomplete entry: " p > "/dev/stderr"}' | \
